...
/* ------------------------- block definitions ------------------------ */
var BLOCKS = {
  stat:    { name: 'Record Count',   group: 'KPI',      desc: 'Count matching Filter Criteria' },
  metric:  { name: 'Metric (MTTR)',  group: 'KPI',      desc: 'avg/sum of a number field' },
  gauge:   { name: 'Ratio Gauge',    group: 'KPI',      desc: 'Two filters, as a percentage' },
  bars:    { name: 'Breakdown Bars', group: 'Analysis', desc: 'Group by -> ranked bars' },
  stacked: { name: 'Distribution',   group: 'Analysis', desc: 'Group by -> one composition bar' },
  donut:   { name: 'Donut Chart',    group: 'Analysis', desc: 'Group by -> donut' },
  list:    { name: 'Record List',    group: 'Records',  desc: 'Clickable rows, sortable' },
...
function defaults(type) {
  ...
  if (type === 'list')    return Object.assign(base, { w: 12, label: 'Triage Queue', subtitle: '', query: emptyQuery(), sortField: '', sortDir: 'ASC', limit: 8, secondaryField: '', showAge: false });
  if (type === 'table')   return Object.assign(base, { w: 12, label: 'Recent Records', columns: 'severity:Severity, status:Status, createDate:Created', query: emptyQuery(), sortField: '', sortDir: 'DESC', limit: 8 });
